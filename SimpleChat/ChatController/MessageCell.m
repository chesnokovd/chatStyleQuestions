...
@interface MessageCell()<UITextFieldDelegate>
...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        if (!_textView) {
            ...
            _textView.layer.rasterizationScale = 2.0f;
...
- (void) setOpponentImage:(UIImage *)opponentImage {
    ...
    if (_sentBy == kSentByUser || !_imageView.image) {
        ...
        for (UIView * v in @[_bgLabel, _textView]) {
...
- (void) drawCell {
    ...
    _textView.text = _message[kMessageContent];
    _sentBy = [_message[kMessageRuntimeSentBy] intValue];
    
    // the height that we want our text bubble to be
    CGFloat height = _textSize.height+20;
    if (height < minimumHeight) height = minimumHeight+10;
    
    if (_sentBy == kSentByUser) {
        // then this is a message that the current user created . . .
        _bgLabel.frame = CGRectMake(ScreenWidth() - offsetX, 0, -self.textSize.width-outlineSpace, height);
        ...
        // sent by opponent
        _bgLabel.frame = CGRectMake(offsetX, 0, self.textSize.width+outlineSpace, height);
        _bgLabel.layer.borderColor = _opponentColor.CGColor;
        _textView.editable=NO;
        _textView.userInteractionEnabled=NO;
    }
    
    // position _textView in the _bgLabel;
    _textView.frame = CGRectMake(_bgLabel.frame.origin.x , 0, _bgLabel.bounds.size.width, _bgLabel.bounds.size.height);
